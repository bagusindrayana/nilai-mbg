<script lang="ts">
  ...
  $: comparisonResults = comparisonItems
    .map((item) => ({
      ...item,
      quantity:
        numericRupiah > 0 ? (numericRupiah / item.price).toFixed(1) : "0",
    }))
...
<div class="border border-gray-300 dark:border-gray-700 p-3">
  ...
  <div class="space-y-3">
    <div class="grid grid-cols-2 gap-2">
      <div class="border-l-2 border-black dark:border-white pl-2">
        ...
        <div class="font-bold text-sm">
          Rp {mbgPriceJava.toLocaleString("id-ID")}
        </div>
      </div>
      <div class="border-l-2 border-black dark:border-white pl-2">
        ...
        <div class="font-bold text-sm">
          Rp {(dailyBudget / 1000000000).toLocaleString("id-ID")} M
        </div>
      </div>
    </div>
    ...
    <div class="border-t border-gray-300 dark:border-gray-700 pt-3">
      ...
      <div class="space-y-2">
        <div>
          <label class="block text-xs font-bold mb-1 uppercase"> Rupiah </label>
          <div class="relative">
            <span
              class="absolute left-2 top-1/2 -translate-y-1/2 text-gray-500 text-sm"
              >Rp</span
            >
            <input
              type="text"
        ...
        <div>
          <label class="block text-xs font-bold mb-1 uppercase"> Porsi </label>
          <input
            type="number"
        ...
        <div>
          <label class="block text-xs font-bold mb-1 uppercase"> Hari </label>
          <input
            type="number"
    ...
    <div class="border-t border-gray-300 dark:border-gray-700 pt-2">
      <div class="text-xs text-gray-500">3 Maret 2026</div>
    </div>
  </div>
</div>
